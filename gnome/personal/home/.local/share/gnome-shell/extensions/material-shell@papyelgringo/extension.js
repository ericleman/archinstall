...
function logAssert(shouldBeTrue, message) {
    if (!shouldBeTrue) {
        GLib$n.log_structured('Material Shell', GLib$n.LogLevelFlags.FLAG_FATAL, {
    ...
    _init({ mouseX, mouseY, size, }) {
        ...
        GLib$m.idle_add(GLib$m.PRIORITY_DEFAULT, () => {
            this.ease({
                ...
                mode: Clutter$r.AnimationMode.EASE_OUT_QUAD,
            });
...
let RippleBackground = class RippleBackground extends St$j.Widget {
    _init(eventListener) {
        ...
        eventListener.connect('event', (actor, event) => {
            ...
            if ([
                Clutter$r.EventType.BUTTON_PRESS,
                Clutter$r.EventType.TOUCH_BEGIN,
            ].indexOf(eventType) > -1) {
                const [_, x, y] = this.transform_stage_point(...event.get_coords());
                this.createRippleWave(x, y);
            }
            else if ([
                Clutter$r.EventType.BUTTON_RELEASE,
                Clutter$r.EventType.TOUCH_END,
                Clutter$r.EventType.LEAVE,
            ].indexOf(eventType) > -1) {
...
let MatButton = class MatButton extends St$i.Widget {
    _init(params) {
        ...
        const clickAction = new Clutter$q.ClickAction();
        clickAction.connect('clicked', (action) => {
            this.clicked = true;
            const button = action.get_button();
            this.emit('clicked', button);
            if (button === Clutter$q.BUTTON_PRIMARY || button === 0) {
                this.emit('primary-action');
            }
            if (button === Clutter$q.BUTTON_SECONDARY) {
        ...
        this.connect('enter-event', () => {
            Me$D.msThemeManager.setCursor(Meta$b.Cursor.POINTING_HAND);
        });
        this.connect('leave-event', () => {
            Me$D.msThemeManager.setCursor(Meta$b.Cursor.DEFAULT);
        });
    }
    _onLongPress(action, actor, state) {
        if (state == Clutter$q.LongPressState.CANCEL) {
            const event = Clutter$q.get_current_event();
            if (this._longPressLater)
                return true;
            this._longPressLater = Meta$b.later_add(Meta$b.LaterType.BEFORE_REDRAW, () => {
        ...
        if (state == Clutter$q.LongPressState.ACTIVATE) {
            this.emit('secondary-action');
        }
        return true;
    }
...
MatButton.metaInfo = {
    ...
    Signals: {
        ...
        'secondary-action': {},
        'drag-start': {
            param_types: [Clutter$q.Event.$gtype],
        },
    },
};
...
let MatPanelButton = class MatPanelButton extends MatButton {
    _init(params = {}) {
        ...
        const panelSizeSignal = Me$C.msThemeManager.connect('panel-size-changed', () => {
            this.queue_relayout();
        });
        this.monitorIndex = this.findMonitor();
        const monitorSignal = Main$l.layoutManager.connect('monitors-changed', () => {
        ...
        this.connect('destroy', () => {
            Me$C.msThemeManager.disconnect(panelSizeSignal);
            Main$l.layoutManager.disconnect(monitorSignal);
        });
    }
    findMonitor() {
        const [x, y] = this.get_transformed_position();
        return (global.display.get_monitor_index_for_rect(new Meta$a.Rectangle({ x, y, width: 0, height: 0 })) || global.display.get_current_monitor());
    }
    vfunc_get_preferred_width(_forHeight) {
        return [
            Me$C.msThemeManager.getPanelSize(this.monitorIndex),
            Me$C.msThemeManager.getPanelSize(this.monitorIndex),
        ];
    }
...
class MsManager extends WithSignals {
    ...
    observe(subject, property, callback) {
        ...
        if (subject instanceof Clutter$p.Actor) {
        ...
        this.signals.forEach((signal) => {
            if (signal.from) {
                try {
                    ...
                    Me$B.log(`Failed to disconnect signal ${signal.id} from ${signal.from} ${signal.from.constructor.name} ${signal.from.toString()}  `);
                }
            }
        });
    }
...
class Async {
    static addTimeout(priority, interval, func) {
        const timeoutId = GLib$k.timeout_add(priority, interval, () => {
            func();
    ...
    static clearTimeoutId(timeoutId) {
        if (timeoutId && this.timeoutIdList.includes(timeoutId)) {
            GLib$k.source_remove(timeoutId);
...
class AsyncDebounce {
    ...
    cancel() {
        if (this.timeoutId !== undefined)
            GLib$k.source_remove(this.timeoutId);
        this.runAgain = false;
    }
    schedule() {
        if (this.running) {
            ...
            this.timeoutId = Async.addTimeout(GLib$k.PRIORITY_DEFAULT, this.delayMs, () => {
...
function throttle(func, wait, options) {
    ...
    return function (...params) {
        ...
        if (remaining <= 0 || remaining > wait) {
            ...
            timeout = Async.addTimeout(GLib$j.PRIORITY_DEFAULT, remaining, later);
        }
        return result;
    };
}
...
const getSettings = (key) => {
    const schema = Gio$e.SettingsSchemaSource.new_from_directory(Me$x.dir.get_child('schemas').get_path(), Gio$e.SettingsSchemaSource.get_default(), false).lookup(Me$x.metadata[key], true);
    if (schema !== null) {
...
class MsThemeManager extends MsManager {
    constructor() {
        ...
        this.themeFile = Gio$d.file_new_for_path(`${GLib$i.get_user_cache_dir()}/${Me$w.uuid}-theme.css`);
        ...
        var displayedCursor = this.metaCursor;
        ...
        this.observe(this.themeContext, 'changed', () => {
            Me$w.log('theme changed');
            this.theme = this.themeContext.get_theme();
            if (Main$j.layoutManager.uiGroup.has_style_class_name('no-theme')) {
                Main$j.layoutManager.uiGroup.remove_style_class_name('no-theme');
            }
            if (!this.theme.application_stylesheet) {
                Main$j.layoutManager.uiGroup.add_style_class_name('no-theme');
            }
        });
    ...
    async writeContentToFile(content, file) {
        return new Promise((resolve, _) => {
            const contentBytes = new GLib$i.Bytes(byteArray.fromString(content));
            file.replace_async(null, false, Gio$d.FileCreateFlags.NONE, GLib$i.PRIORITY_DEFAULT, null, (file, res) => {
                const stream = assertNotNull(file).replace_finish(res);
                stream.write_bytes_async(contentBytes, GLib$i.PRIORITY_DEFAULT, null, (ioStream, wRes) => {
    ...
    async buildThemeStylesheetToFile(file) {
        const originThemeFile = Gio$d.file_new_for_path(`${Me$w.path}/style-${this.themeValue}-theme.css`);
...
const { main: Main$i } = imports.ui;
function getExtensionSettings(extensionUUID) {
    const extension = Main$i.extensionManager.lookup(extensionUUID);
    if (!extension)
        return null;
    try {
        return new Gio$c.Settings({
    ...
    try {
        const source = Gio$c.SettingsSchemaSource.new_from_directory(extension.path + '/schemas/', Gio$c.SettingsSchemaSource.get_default(), false);
        return new Gio$c.Settings({
...
const { dateMenu, main: Main$h, panel } = imports.ui;
const Me$v = imports.misc.extensionUtils.getCurrentExtension();
let MsStatusArea = class MsStatusArea extends Clutter$o.Actor {
    _init() {
        super._init({
            layout_manager: new Clutter$o.BoxLayout({
                orientation: Clutter$o.Orientation.VERTICAL,
                pack_start: false,
            }),
        });
        this.signalIds = null;
        this.originalAppIndicatorIconSize = undefined;
        this.gnomeShellPanel = Main$h.panel;
        this.leftBoxActors = [];
        this.centerBoxActors = [];
        this.rightBoxActors = [];
        this.dateMenu = this.gnomeShellPanel.statusArea.dateMenu;
        this.enable();
        const panelSizeSignal = Me$v.msThemeManager.connect('panel-size-changed', () => this.onPanelSizeChanged());
        this.connect('destroy', () => {
            Me$v.msThemeManager.disconnect(panelSizeSignal);
    ...
    iconSize() {
        return Math.round(Me$v.msThemeManager.getPanelSizeNotScaled() * (16.0 / 48.0));
    }
    recursivelySetProperties(actor, controlledByMS) {
        if (actor instanceof St$g.BoxLayout) {
            actor.vertical = controlledByMS;
            actor.set_x_align(Clutter$o.ActorAlign.CENTER);
        }
        ...
        if (actor instanceof Shell$8.TrayIcon) {
            if (controlledByMS) {
                const iconSize = this.iconSize();
        ...
        for (const menuData of this.gnomeShellPanel.menuManager._menus) {
            ...
            if (menu._boxPointer) {
                ...
                menu._boxPointer._calculateArrowSide = function () {
                    return Me$v.msThemeManager.verticalPanelPosition ===
    ...
    disable() {
        Me$v.logFocus('disable statusArea');
...
let MsDateMenuBox = class MsDateMenuBox extends St$g.Widget {
    _init(dateMenu) {
        super._init({
            x_align: Clutter$o.ActorAlign.CENTER,
            layout_manager: new Clutter$o.BinLayout(),
        });
        ...
        this.notificationIcon = new St$g.Icon({
            gicon: Gio$b.icon_new_for_string(`${Me$v.path}/assets/icons/bell-symbolic.svg`),
        });
        this.notificationIconRing = new St$g.Icon({
            style_class: 'primary',
            gicon: Gio$b.icon_new_for_string(`${Me$v.path}/assets/icons/bell-ring-symbolic.svg`),
        });
        this.dndIcon = new St$g.Icon({
            gicon: Gio$b.icon_new_for_string(`${Me$v.path}/assets/icons/bell-off-symbolic.svg`),
        });
        this._settings = new Gio$b.Settings({
            schema_id: 'org.gnome.desktop.notifications',
        });
        this.iconDisplay = new Clutter$o.Actor();
        ...
        if (Me$v.msThemeManager.clockHorizontal) {
            this.add_child(this.iconDisplay);
        }
        else {
            this.add_child(this.clockLabel);
        }
        const clockHorizontalSignal = Me$v.msThemeManager.connect('clock-horizontal-changed', () => {
            if (Me$v.msThemeManager.clockHorizontal) {
        ...
        this.connect('destroy', () => {
            Me$v.msThemeManager.disconnect(clockHorizontalSignal);
...
function centerInBox(box, width, height) {
    ...
    return Clutter$n.ActorBox.new(x, y, x + width, y + height);
}
...
let AppPlaceholder = class AppPlaceholder extends St$f.Widget {
    _init(app) {
        super._init({
            x_align: Clutter$m.ActorAlign.FILL,
            y_align: Clutter$m.ActorAlign.FILL,
        ...
        assert(icon instanceof St$f.Widget, 'expected icon to be a widget');
        this.icon = icon;
    ...
    vfunc_key_press_event(keyEvent) {
        switch (keyEvent.keyval) {
            ...
            case Clutter$m.KEY_space:
            case Clutter$m.KEY_KP_Space:
                this.activate(0);
...
const { dialog, main: Main$g } = imports.ui;
...
let MsWindow = class MsWindow extends Clutter$l.Actor {
    _init({ app, persistent, initialAllocation, msWorkspace, lifecycleState, }) {
        super._init({
            reactive: true,
    ...
    delayGetMetaWindowActor(metaWindow, delayedCount, resolve, reject) {
        if (delayedCount < 20) {
            Async.addTimeout(GLib$g.PRIORITY_DEFAULT, 50, () => {
                const actor = metaWindow.get_compositor_private();
    ...
    get dragged() {
        var _a;
        return (((_a = Me$s.msWindowManager.msDndManager.dragInProgress) === null || _a === void 0 ? void 0 : _a.msWindow) === this);
    }
    ...
    mimicMetaWindowPositionAndSize() {
        ...
        if (!metaWindow || this.dragged)
            return;
        const workArea = Main$g.layoutManager.getWorkAreaForMonitor(metaWindow.get_monitor());
        const currentFrameRect = metaWindow.get_frame_rect();
        const newPosition = {
            x: currentFrameRect.x -
    ...
    updateWorkspaceAndMonitor(metaWindow) {
        if (metaWindow && this.msWorkspace) {
            ...
            const workspace = Me$s.msWorkspaceManager.getWorkspaceOfMsWorkspace(this.msWorkspace);
    ...
    grab_key_focus() {
        this.focusDialogs();
        if (!Me$s.msWindowManager.msFocusManager.requestFocus(this))
            return;
        if (this.metaWindow) {
    ...
    metaWindowUnManaged(metaWindow) {
        ...
        if (!isMainMetaWindow && !dialog) {
            Me$s.log('Cannot find the window which was unmanaged');
            return;
        }
        if (dialog) {
    ...
    updateMetaWindowVisibility() {
        ...
        if (metaWindow) {
            const shouldBeHidden = !this.visible ||
                ...
                Me$s.msWindowManager.msDndManager.dragInProgress;
    ...
    vfunc_allocate(box, flags) {
        ...
        if (metaWindow &&
            ...
            if (metaWindow.resizeable || metaWindow.fullscreen) {
                ...
                const workArea = Main$g.layoutManager.getWorkAreaForMonitor(monitor.index);
...
class IdleDebounce {
    ...
    schedule(...args) {
        if (this.scheduleInfo === null) {
            this.scheduleInfo = {
                signal: GLib$f.idle_add(GLib$f.PRIORITY_DEFAULT_IDLE, () => {
    ...
    cancel() {
        if (this.scheduleInfo !== null) {
            GLib$f.Source.remove(this.scheduleInfo.signal);
            this.scheduleInfo = null;
        }
    }
}
...
class AppsManager {
    static getApps() {
        const usage = Shell$7.AppUsage.get_default();
        const appSystem = Shell$7.AppSystem.get_default();
...
const { main: Main$f } = imports.ui;
const Me$r = imports.misc.extensionUtils.getCurrentExtension();
const BUTTON_SIZE = 124;
let MsApplicationLauncher = class MsApplicationLauncher extends St$d.Widget {
    _init(msWorkspace) {
        super._init({
            reactive: true,
            style: 'padding:64px',
        ...
        this.launcherChangedSignal = SignalHandle.connect(Me$r.msThemeManager, 'clock-app-launcher-changed', () => {
            this.restartAppListContainer();
        });
        this.installedChangedSignal = SignalHandle.connect(Shell$6.AppSystem.get_default(), 'installed-changed', () => {
            this.restartAppListContainer();
        });
        this.connect('key-focus-in', () => {
    ...
    initAppListContainer() {
        ...
        AppsManager.getApps().forEach((app) => {
            ...
            button.connect('clicked', () => {
                Me$r.msWindowManager.openApp(app, this.msWorkspace);
    ...
    vfunc_allocate(box, flags) {
        ...
        const themeNode = this.get_theme_node();
        const contentBox = themeNode.get_content_box(box);
        const containerBox = new Clutter$k.ActorBox();
        const minSize = Math.min(contentBox.get_width(), contentBox.get_height());
        const workArea = Main$f.layoutManager.getWorkAreaForMonitor(this.msWorkspace.monitor.index);
...
let MsApplicationButtonContainer = class MsApplicationButtonContainer extends St$d.Widget {
    _init(msWorkspace) {
        super._init({});
        this.msWorkspace = msWorkspace;
        this.appButtonList = [];
        ...
        this.maxIndex = 1;
        if (Me$r.msThemeManager.clockAppLauncher) {
            const clockLabel = new St$d.Label({
                style_class: 'headline-6 text-medium-emphasis margin-right-x2',
                y_align: Clutter$k.ActorAlign.CENTER,
            });
            this.clockLabel = clockLabel;
            const dateLabel = new St$d.Label({
                style_class: 'headline-6 text-medium-emphasis',
                y_align: Clutter$k.ActorAlign.CENTER,
            });
            this.dateLabel = dateLabel;
            this.clockBin = new St$d.BoxLayout({
                x_align: Clutter$k.ActorAlign.CENTER,
            });
            ...
            const updateClock = () => {
                if (clockLabel.mapped) {
                    ...
                    const date = new Date();
                    const dateFormat = Shell$6.util_translate_time_string(N_('%A %B %-d'));
        ...
        this.inputLayout = new St$d.BoxLayout({});
        this.searchIcon = new St$d.Icon({
            style_class: 'search-entry-icon',
            icon_name: 'edit-find-symbolic',
        });
        this.inputContainer = new St$d.Entry({
        ...
        this._text.connect('key-press-event', (entry, event) => {
            ...
            switch (symbol) {
                case Clutter$k.KEY_Escape:
                    this.reset();
                    this.removeHighlightButton();
                    return Clutter$k.EVENT_STOP;
                case Clutter$k.KEY_Tab:
                    this.highlightNextButton();
                    return Clutter$k.EVENT_STOP;
                case Clutter$k.KEY_ISO_Left_Tab:
                    this.highlightPreviousButton();
                    return Clutter$k.EVENT_STOP;
                case Clutter$k.KEY_Down:
                    this.highlightButtonBelow();
                    return Clutter$k.EVENT_STOP;
                case Clutter$k.KEY_Up:
                    this.highlightButtonAbove();
                    return Clutter$k.EVENT_STOP;
                case Clutter$k.KEY_Right:
                    if (this._text.cursor_position === -1) {
                        this.highlightNextButton();
                        return Clutter$k.EVENT_STOP;
                    }
                    else {
                        return Clutter$k.EVENT_PROPAGATE;
                    }
                case Clutter$k.KEY_Left:
                ...
                case Clutter$k.KEY_Return:
                case Clutter$k.KEY_KP_Enter:
                    if (this.currentButtonFocused)
        ...
        this.container = new St$d.Widget();
    ...
    vfunc_allocate(box, flags) {
        ...
        const clockHeight = (Me$r.msThemeManager.clockAppLauncher ? 64 : 0) * this.monitorScale;
        ...
        const verticalOffset = (contentBox.get_height() -
            ...
            2;
        if (this.clockBin) {
            const clockBox = new Clutter$k.ActorBox();
        ...
        const searchBox = new Clutter$k.ActorBox();
        ...
        for (let y = 0; y < this.numberOfRow; y++) {
            for (let x = 0; x < this.numberOfColumn; x++) {
                ...
                if (index < this.filteredAppButtonList.length) {
                    ...
                    const buttonBox = new Clutter$k.ActorBox();
                    buttonBox.x1 =
                        containerBox.x1 +
                            this.buttonSize * x +
                            containerPadding;
        ...
        const hiddenBox = new Clutter$k.ActorBox();
...
let MsApplicationButton = class MsApplicationButton extends MatButton {
    _init({ app, buttonSize }) {
        ...
        this.layout = new St$d.BoxLayout({
            vertical: true,
        ...
        if (app) {
            ...
            Me$r.tooltipManager.add(this.title, { relativeActor: this });
        }
...
let ReorderableList = class ReorderableList extends Clutter$j.Actor {
    _init(vertical = false, classAccepted = []) {
        super._init({
            layout_manager: new Clutter$j.BoxLayout({
                orientation: vertical
                    ? Clutter$j.Orientation.VERTICAL
                    : Clutter$j.Orientation.HORIZONTAL,
            }),
        });
        this.vertical = vertical;
...
ReorderableList.metaInfo = {
    ...
    Signals: {
        ...
        'drag-end': {},
        'actor-moved': {
            param_types: [Clutter$j.Actor.$gtype, GObject$4.TYPE_INT],
        },
...
let DropPlaceholder = class DropPlaceholder extends St$c.Widget {
    _init() {
...
DropPlaceholder.metaInfo = {
    ...
    Signals: {
        'drag-dropped': {
            param_types: [Clutter$j.Actor.$gtype],
        },
        'drag-over': {},
    },
};
...
let TaskBar = class TaskBar extends St$b.Widget {
    _init(msWorkspace, panelMenuManager) {
        ...
        this.taskButtonContainer.connect('foreign-actor-inserted', (_, actor, index) => {
            if (actor.tileable instanceof MsWindow) {
                Me$p.msWorkspaceManager.setWindowToMsWorkspace(actor.tileable, this.msWorkspace);
                this.msWorkspace.setTileableAtIndex(actor.tileable, index);
                this.msWorkspace.focusTileable(actor.tileable);
                Me$p.msWorkspaceManager.stateChanged();
            }
        });
        ...
        this.tracker = Shell$5.WindowTracker.get_default();
    ...
    createNewItemForTileable(tileable) {
        ...
        if (tileable instanceof MsWindow) {
            ...
            item = new IconTaskBarItem(tileable, Gio$a.icon_new_for_string(`${Me$p.path}/assets/icons/plus-symbolic.svg`));
        }
    ...
    vfunc_allocate(box, flags) {
        ...
        if (activeItem) {
            this.taskActiveIndicator.show();
            const taskActiveIndicatorBox = new Clutter$i.ActorBox({
                x1: activeItem.x,
...
let TaskActiveIndicator = class TaskActiveIndicator extends St$b.Widget {
    _init(...args) {
        super._init(...args);
    }
    ...
    vfunc_allocate(...args) {
        if (this.width && this.mapped) {
            ...
            GLib$e.idle_add(GLib$e.PRIORITY_DEFAULT_IDLE, () => {
                this.animate();
...
let TaskBarItem = class TaskBarItem extends MatButton {
    _init(contentActor, draggable) {
        ...
        this.monitor = assertNotNull(Main$e.layoutManager.primaryMonitor);
    ...
    vfunc_parent_set() {
        ...
        if (actor.is_mapped()) {
            this.monitor = assertNotNull(Main$e.layoutManager.findMonitorForActor(actor));
        }
    }
    vfunc_get_preferred_height(_forWidth) {
        const height = Me$p.msThemeManager.getPanelSize(this.monitor.index);
...
let TileableItem = class TileableItem extends TaskBarItem {
    _init(tileable) {
        const container = new St$b.BoxLayout({
        ...
        if (ShellVersionMatch('3.34')) {
            this.startIconContainer = new St$b.Bin({
                y_align: 1,
            });
        }
        else {
            this.startIconContainer = new St$b.Bin({
                y_align: Clutter$i.ActorAlign.CENTER,
            });
        }
        if (ShellVersionMatch('3.34')) {
            this.endIconContainer = new St$b.Bin({
                y_align: 1,
            });
        }
        else {
        ...
        this.menu = new PopupMenu$1.PopupMenu(this, 0.5, St$b.Side.TOP);
        ...
        }, Gio$a.icon_new_for_string(`${Me$p.path}/assets/icons/pin-symbolic.svg`));
        ...
        }, Gio$a.icon_new_for_string(`${Me$p.path}/assets/icons/pin-off-symbolic.svg`));
        this.menu.addAction('Close', () => {
            this.emit('close-clicked');
        }, Gio$a.icon_new_for_string(`${Me$p.path}/assets/icons/close-symbolic.svg`));
        Main$e.layoutManager.uiGroup.add_actor(this.menu.actor);
        this.menu.actor.hide();
        this.title = new St$b.Label({
            style_class: 'task-bar-item-title',
            y_align: Clutter$i.ActorAlign.CENTER,
        });
        Me$p.tooltipManager.add(this.title, { relativeActor: this });
        ...
        this.connect('destroy', this._onDestroy.bind(this));
        this.closeIcon = new St$b.Icon({
            style_class: 'task-small-icon',
            gicon: Gio$a.icon_new_for_string(`${Me$p.path}/assets/icons/close-symbolic.svg`),
        });
        this.closeButton = new St$b.Button({
        ...
        this.persistentIcon = new St$b.Icon({
            style_class: 'task-small-icon',
            gicon: Gio$a.icon_new_for_string(`${Me$p.path}/assets/icons/pin-symbolic.svg`),
        });
    ...
    buildIcon(height) {
        ...
        assert(icon instanceof St$b.Widget, 'expected icon to be a widget');
        this.icon = icon;
        ...
        if (this.tileable instanceof MsApplicationLauncher ||
            ...
            if (this.style == 'full') {
                if (this.tileable.title.includes(this.app.get_name())) {
                    ...
                    const escapedAppName = GLib$e.markup_escape_text(this.app.get_name(), -1);
                    const escapedTitle = GLib$e.markup_escape_text(this.tileable.title, -1);
...
let IconTaskBarItem = class IconTaskBarItem extends TaskBarItem {
    _init(tileable, gicon) {
        ...
        this.icon = new St$b.Icon({
            gicon,
            style_class: 'app-icon',
            icon_size: Me$p.msThemeManager.getPanelSizeNotScaled() / 2,
        });
...
class MsWorkspaceCategory {
    ...
    forceCategory(category) {
        ...
        Me$o.stateManager.stateChanged();
    }
    refreshCategory() {
        this.category = this.determineCategory();
    }
...
const { main: Main$d, popupMenu: popupMenu$1 } = imports.ui;
const DND = imports.ui.dnd;
const Me$n = imports.misc.extensionUtils.getCurrentExtension();
let WorkspaceList = class WorkspaceList extends St$a.Widget {
    _init() {
        super._init({
            clip_to_allocation: true,
        ...
        this.msWorkspaceManager = Me$n.msWorkspaceManager;
        ...
        this.workspaceActiveIndicator = new St$a.Widget({
            style_class: 'workspace-active-indicator',
            height: Me$n.msThemeManager.getPanelSize(Main$d.layoutManager.primaryIndex),
        });
        const panelSizeSignal = Me$n.msThemeManager.connect('panel-size-changed', () => {
            this.workspaceActiveIndicator.set_height(Me$n.msThemeManager.getPanelSize(Main$d.layoutManager.primaryIndex));
            this.queue_relayout();
        });
        ...
        this.connect('scroll-event', (_, event) => {
            switch (event.get_scroll_direction()) {
                case Clutter$h.ScrollDirection.UP:
                    this.msWorkspaceManager.activatePreviousMsWorkspace();
                    break;
                case Clutter$h.ScrollDirection.DOWN:
    ...
    activeButtonForIndex(index) {
        if (this.buttonActive) {
            ...
            assert(child instanceof St$a.Widget, 'Child was not a widget');
        ...
        this.workspaceActiveIndicator.ease({
            ...
            duration: 250,
            mode: Clutter$h.AnimationMode.EASE_OUT_QUAD,
        });
    }
    vfunc_get_preferred_width(_forHeight) {
        return [
            Me$n.msThemeManager.getPanelSize(Main$d.layoutManager.primaryIndex),
            Me$n.msThemeManager.getPanelSize(Main$d.layoutManager.primaryIndex),
        ];
    }
    _onDestroy() {
...
let WorkspaceButton = class WorkspaceButton extends MatButton {
    _init(msWorkspaceManager, msWorkspace) {
        ...
        this.connect('clicked', (actor, button) => {
            if (button === Clutter$h.BUTTON_MIDDLE) {
    ...
    buildMenu() {
        const rootMenu = new popupMenu$1.PopupMenu(this, 0.5, St$a.Side.LEFT);
        ...
        const panelIconStyleHybridRadio = rootMenu.addAction(_('Hybrid'), () => {
            Me$n.msThemeManager.panelIconStyle = PanelIconStyleEnum.HYBRID;
        }, Gio$9.icon_new_for_string(`${Me$n.path}/assets/icons/radiobox-${Me$n.msThemeManager.panelIconStyle ===
            PanelIconStyleEnum.HYBRID
            ? 'marked'
            : 'blank'}-symbolic.svg`));
        const panelIconStyleCategoryRadio = rootMenu.addAction(_('Categories only'), () => {
            Me$n.msThemeManager.panelIconStyle = PanelIconStyleEnum.CATEGORY;
        }, Gio$9.icon_new_for_string(`${Me$n.path}/assets/icons/radiobox-${Me$n.msThemeManager.panelIconStyle ===
            PanelIconStyleEnum.CATEGORY
            ? 'marked'
            : 'blank'}-symbolic.svg`));
        const panelIconStyleApplicationRadio = rootMenu.addAction(_('Applications preview'), () => {
            Me$n.msThemeManager.panelIconStyle =
                PanelIconStyleEnum.APPLICATION;
        }, Gio$9.icon_new_for_string(`${Me$n.path}/assets/icons/radiobox-${Me$n.msThemeManager.panelIconStyle ===
            PanelIconStyleEnum.APPLICATION
            ? 'marked'
            : 'blank'}-symbolic.svg`));
        const iconStyleSignal = Me$n.msThemeManager.connect('panel-icon-style-changed', () => {
            assertNotNull(panelIconStyleHybridRadio._icon).set_gicon(Gio$9.icon_new_for_string(`${Me$n.path}/assets/icons/radiobox-${Me$n.msThemeManager.panelIconStyle ===
                PanelIconStyleEnum.HYBRID
                ? 'marked'
                : 'blank'}-symbolic.svg`));
            assertNotNull(panelIconStyleCategoryRadio._icon).set_gicon(Gio$9.icon_new_for_string(`${Me$n.path}/assets/icons/radiobox-${Me$n.msThemeManager.panelIconStyle ===
                PanelIconStyleEnum.CATEGORY
                ? 'marked'
                : 'blank'}-symbolic.svg`));
            assertNotNull(panelIconStyleApplicationRadio._icon).set_gicon(Gio$9.icon_new_for_string(`${Me$n.path}/assets/icons/radiobox-${Me$n.msThemeManager.panelIconStyle ===
                PanelIconStyleEnum.APPLICATION
                ? 'marked'
                : 'blank'}-symbolic.svg`));
        });
        ...
        MainCategories.forEach((key) => {
            subMenu.menu.addAction(key, () => {
                setCategory(key);
            }, Gio$9.icon_new_for_string(`${Me$n.path}/assets/icons/category/${key.toLowerCase()}-symbolic.svg`));
        });
        rootMenu.addMenuItem(subMenu);
        Main$d.layoutManager.uiGroup.add_actor(rootMenu.actor);
        rootMenu.close();
        return {
            menu: rootMenu,
            ...
            subMenu,
            disconnectSignals: () => {
                Me$n.msThemeManager.disconnect(iconStyleSignal);
            },
        };
    }
    initDrag() {
    ...
    acceptDrop(source) {
        if (source instanceof TaskBarItem) {
            if (source.tileable instanceof MsWindow) {
                Me$n.msWorkspaceManager.setWindowToMsWorkspace(source.tileable, this.msWorkspace);
...
let WorkspaceButtonIcon = class WorkspaceButtonIcon extends St$a.Widget {
    _init(msWorkspace) {
        super._init();
        this.msWorkspace = msWorkspace;
        this.appIconList = [];
        ...
        const themeSignals = [
            Me$n.msThemeManager.connect('panel-icon-style-changed', () => {
                this.buildIcons();
            }),
            Me$n.msThemeManager.connect('panel-icon-color-changed', () => {
                this.desaturateIcons();
            }),
            Me$n.msThemeManager.connect('panel-size-changed', () => {
                this.buildIcons();
            }),
        ];
        this.connect('destroy', () => {
            for (const s of themeSignals)
                Me$n.msThemeManager.disconnect(s);
        });
    }
    desaturateIcons() {
        const shouldDesaturate = !Me$n.msThemeManager.panelIconColor;
        ...
        if (shouldDesaturate === isDesaturate)
            return;
        if (shouldDesaturate) {
            this.desaturateEffect = new Clutter$h.DesaturateEffect();
    ...
    buildIcons() {
        ...
        if (appList.length) {
            ...
            if (this.msWorkspace.msWorkspaceCategory.forcedCategory ||
                ...
                const icon = new St$a.Icon({
                    gicon: Gio$9.icon_new_for_string(`${Me$n.path}/assets/icons/category/${category.toLowerCase()}-symbolic.svg`),
                    icon_size: Me$n.msThemeManager.getPanelSizeNotScaled() / 2,
                });
                ...
                sortedByInstanceAppList.forEach((app) => {
                    const icon = app.create_icon_texture(Me$n.msThemeManager.getPanelSizeNotScaled() / 2);
                    this.appIconList.push(icon);
                    this.add_child(icon);
                });
            }
            ...
            const icon = new St$a.Icon({
                gicon: Gio$9.icon_new_for_string(`${Me$n.path}/assets/icons/plus-symbolic.svg`),
                icon_size: Me$n.msThemeManager.getPanelSizeNotScaled() / 2,
            });
            this.appIconList.push(icon);
            this.add_child(icon);
    ...
    vfunc_allocate(allocationBox, flags) {
        ...
        if (this.appIconList.length === 1) {
            const centerBox = new Clutter$h.ActorBox();
            ...
            this.appIconList.forEach((icon, index) => {
                const box = new Clutter$h.ActorBox();
                switch (index) {
                    case 0:
...
let MatDivider = class MatDivider extends St$9.Widget {
...
const { appDisplay, remoteSearch } = imports.ui;
...
let SearchResultHeader = class SearchResultHeader extends St$8.Bin {
    _init(text) {
        super._init({
            style_class: 'subtitle-2 margin margin-top-x2 margin-bottom-x2 text-high-emphasis',
        });
        this.label = new St$8.Label({
            text: text,
        });
...
let SearchResultEntry = class SearchResultEntry extends MatButton {
    _init(icon, title, description, _withMenu) {
        super._init({});
        this.layout = new St$8.BoxLayout();
        this.textLayout = new St$8.BoxLayout({
            vertical: true,
            styleClass: 'margin-left-x2 margin-top margin-bottom margin-right-x2',
            y_align: Clutter$g.ActorAlign.CENTER,
        });
        if (icon) {
            this.icon = icon;
        ...
        this.title = new St$8.Label({
            text: title,
        });
        this.textLayout.add_child(this.title);
        if (description) {
            this.description = new St$8.Label({
                text: description,
                styleClass: 'caption',
...
let SearchResultList = class SearchResultList extends St$8.BoxLayout {
    _init(searchEntry) {
        super._init({
    ...
    onKeyPress(entry, event) {
        const symbol = event.get_key_symbol();
        if (symbol === Clutter$g.KEY_Escape) {
            this.resetAndClose();
            return Clutter$g.EVENT_STOP;
        }
        else {
            if (symbol === Clutter$g.KEY_Tab) {
                this.selectNext();
                return Clutter$g.EVENT_STOP;
            }
            else if (symbol === Clutter$g.KEY_ISO_Left_Tab) {
                this.selectPrevious();
                return Clutter$g.EVENT_STOP;
            }
            else if (symbol === Clutter$g.KEY_Down) {
                this.selectNext();
                return Clutter$g.EVENT_STOP;
            }
            else if (symbol === Clutter$g.KEY_Up) {
                this.selectPrevious();
                return Clutter$g.EVENT_STOP;
            }
            else if (symbol === Clutter$g.KEY_Return ||
                symbol === Clutter$g.KEY_KP_Enter) {
        ...
        return Clutter$g.EVENT_PROPAGATE;
    }
    doSearch() {
        this.startingSearch = false;
    ...
    setTerms(terms) {
        ...
        if (this.searchTimeoutId == 0)
            this.searchTimeoutId = Async.addTimeout(GLib$d.PRIORITY_DEFAULT, 150, this.onSearchTimeout.bind(this));
    }
    ...
    updateResults(provider, results) {
        ...
        const onSearchMetas = (resMetas) => {
            let moreEntry = null;
            const extraResults = [];
            if (resMetas.length > 5) {
                const more = (moreEntry = new SearchResultEntry(new St$8.Icon({
                    icon_size: 32,
                    gicon: Gio$8.icon_new_for_string(`${Me$l.path}/assets/icons/chevron-down-symbolic.svg`),
            ...
            for (const res of resMetas) {
                ...
                entry.connect('primary-action', () => {
                    ...
                    if (provider.isRemoteProvider) {
                        ...
                        const app = Shell$4.AppSystem.get_default().lookup_app(res.id);
                        if (app) {
                            Me$l.msWindowManager.openApp(app, Me$l.msWorkspaceManager.getActiveMsWorkspace());
                        }
                        else {
    ...
    selectNext() {
        const currentIndex = this.entrySelected !== null
...
let PanelContent = class PanelContent extends St$7.BoxLayout {
    _init() {
        ...
        this.buttonIcon = new St$7.Icon({
            ...
            icon_size: Me$k.msThemeManager.getPanelSizeNotScaled() / 2,
        });
        this.searchButton = new MatPanelButton({
            child: this.buttonIcon,
            primary: true,
            height: Math.max(48, Me$k.msThemeManager.getPanelSize(Main$c.layoutManager.primaryIndex)),
        });
        ...
        const panelSizeSignal = Me$k.msThemeManager.connect('panel-size-changed', () => {
            this.buttonIcon.set_icon_size(Me$k.msThemeManager.getPanelSizeNotScaled() / 2);
            this.searchButton.height = Math.max(48, Me$k.msThemeManager.getPanelSize(Main$c.layoutManager.primaryIndex));
            this.queue_relayout();
        });
        this.connect('destroy', () => {
            Me$k.msThemeManager.disconnect(panelSizeSignal);
        });
        this.setIcon('search');
    }
    enable() {
    ...
    vfunc_get_preferred_width(_for_height) {
        const panelSize = Me$k.msThemeManager.getPanelSize(Main$c.layoutManager.primaryIndex);
        return [panelSize, panelSize];
    }
    setIcon(icon) {
        if (icon === 'search') {
            this.buttonIcon.set_gicon(Gio$7.icon_new_for_string(`${Me$k.path}/assets/icons/magnify-symbolic.svg`));
        }
        if (icon === 'close') {
            this.buttonIcon.set_gicon(Gio$7.icon_new_for_string(`${Me$k.path}/assets/icons/close-symbolic.svg`));
        }
    }
};
...
let SearchContent = class SearchContent extends St$7.BoxLayout {
    _init() {
        super._init({
            vertical: true,
            y_expand: true,
            x_expand: true,
            x_align: Clutter$f.ActorAlign.FILL,
        });
        this.scrollView = new St$7.ScrollView({
            x_expand: true,
            hscrollbar_policy: St$7.PolicyType.NEVER,
            vscrollbar_policy: St$7.PolicyType.AUTOMATIC,
        });
        this.searchEntry = new St$7.Entry({
            style_class: 'search-entry margin',
            hint_text: _('Type to search'),
            track_hover: true,
            can_focus: true,
        });
        this.searchEntry.set_offscreen_redirect(Clutter$f.OffscreenRedirect.ALWAYS);
        this.searchEntryBin = new St$7.Bin({
        ...
        this.searchResultList.connect('result-selected-changed', (_, res) => {
            Me$k.logFocus('new result', res);
            Util.ensureActorVisibleInScrollView(this.scrollView, res);
        });
        this.scrollView.add_actor(this.searchResultList);
        const panelSizeSignal = Me$k.msThemeManager.connect('panel-size-changed', () => {
            this.searchEntryBin.height = Math.max(48, Me$k.msThemeManager.getPanelSize(Main$c.layoutManager.primaryIndex));
            this.queue_relayout();
        });
        this.connect('destroy', () => {
            Me$k.msThemeManager.disconnect(panelSizeSignal);
    ...
    vfunc_get_preferred_width(_forHeight) {
        return [
            448 -
                Me$k.msThemeManager.getPanelSize(Main$c.layoutManager.primaryIndex),
            448 -
                Me$k.msThemeManager.getPanelSize(Main$c.layoutManager.primaryIndex),
        ];
    }
...
let MsPanel = class MsPanel extends St$7.BoxLayout {
    _init() {
        ...
        this.disableConnect = Me$k.connect('extension-disable', () => {
            Me$k.logFocus('extension-disable');
            Me$k.disconnect(this.disableConnect);
            this.disable();
        });
        const panelSizeSignal = Me$k.msThemeManager.connect('panel-size-changed', () => {
            this.queue_relayout();
    ...
    toggle() {
        if (!this.isExpanded) {
            if (!Me$k.layout.panelsVisible) {
                this.show();
            }
            if (this.searchContent.get_parent() === null) {
                if (Me$k.msThemeManager.verticalPanelPosition ===
                    VerticalPanelPositionEnum.LEFT) {
            ...
            this.translation_x =
                (448 -
                    (Me$k.layout.panelsVisible
                        ? Me$k.msThemeManager.getPanelSize(Main$c.layoutManager.primaryIndex)
                        : 0)) *
                    (Me$k.msThemeManager.verticalPanelPosition ===
                        VerticalPanelPositionEnum.LEFT
                        ? -1
                        : 1);
            this.ease({
...
let TranslationAnimator = class TranslationAnimator extends Clutter$e.Actor {
    _init(vertical = false) {
        super._init({
        ...
        this.vertical = vertical;
        this.transitionContainer = new Clutter$e.Actor({
            layout_manager: new Clutter$e.BoxLayout({
                orientation: this.vertical
                    ? Clutter$e.Orientation.VERTICAL
                    : Clutter$e.Orientation.HORIZONTAL,
            }),
        });
        this.add_actor(this.transitionContainer);
    }
    tryRemoveActor(actor) {
...
let MsMain = class MsMain extends St$6.Widget {
    _init() {
        ...
        Me$i.layout = this;
        this.panelsVisible = (_a = Me$i.stateManager.getState('panels-visible')) !== null && _a !== void 0 ? _a : true;
        Main$b.layoutManager.uiGroup.insert_child_above(this, global.window_group);
        this.monitorsContainer = [];
        this.aboveContainer = new Clutter$d.Actor();
        this.add_child(this.aboveContainer);
        this.backgroundGroup = new Meta$7.BackgroundGroup({});
        this.setBlurBackground(Me$i.msThemeManager.blurBackground);
        ...
        this.primaryMonitorContainer.setMsWorkspaceActor(Me$i.msWorkspaceManager.getActivePrimaryMsWorkspace().msWorkspaceActor);
        ...
        this.signals.push({
            ...
            id: this.connect('captured-event', (_, event) => {
                if (!this.overviewShown)
                    return;
                if (event.type() === Clutter$d.EventType.BUTTON_PRESS) {
                ...
                return Clutter$d.EVENT_PROPAGATE;
            }),
        });
        this.signals.push({
            from: Me$i.msThemeManager,
            id: Me$i.msThemeManager.connect('blur-background-changed', () => {
                this.setBlurBackground(Me$i.msThemeManager.blurBackground);
            }),
        });
        this.signals.push({
            from: Me$i.msWorkspaceManager,
            id: Me$i.msWorkspaceManager.connect('switch-workspace', (_, from, to) => {
                this.onSwitchWorkspace(from, to);
            }),
        });
        this.signals.push({
            from: Me$i.msWorkspaceManager,
            id: Me$i.msWorkspaceManager.connect('dynamic-super-workspaces-changed', () => {
                this.onMsWorkspacesChanged();
            }),
        });
        this.signals.push({
            from: Me$i,
            id: Me$i.connect('extension-disable', () => {
    ...
    onMsWorkspacesChanged() {
        ...
        this.monitorsContainer.forEach((container) => {
            const msWorkspace = Me$i.msWorkspaceManager.getMsWorkspacesOfMonitorIndex(container.monitor.index)[0];
            if (msWorkspace) {
    ...
    togglePanelsVisibilities() {
        ...
        Me$i.stateManager.setState('panels-visible', this.panelsVisible);
        this.updatePanelVisibilities();
    }
    updatePanelVisibilities() {
        ...
        Me$i.msWorkspaceManager.refreshMsWorkspaceUI();
    }
    ...
    toggleOverview() {
        if (this.overviewShown) {
            this.overviewShown = false;
            this.primaryMonitorContainer.workspaceContainer.ease_property('@effects.dimmer.brightness', Clutter$d.Color.new(127, 127, 127, 255), {
                duration: 300,
            ...
            Me$i.msWindowManager.msFocusManager.popModal(this);
        }
        else {
            this.overviewShown = true;
            Me$i.msWindowManager.msFocusManager.pushModal(this, {
                actionMode: Shell$3.ActionMode.OVERVIEW,
            });
            const dimmerEffect = new Clutter$d.BrightnessContrastEffect({
                name: 'dimmer',
                brightness: Clutter$d.Color.new(127, 127, 127, 255),
            });
            this.primaryMonitorContainer.workspaceContainer.add_effect(dimmerEffect);
            this.primaryMonitorContainer.workspaceContainer.ease_property('@effects.dimmer.brightness', Clutter$d.Color.new(90, 90, 90, 255), {
                duration: 300,
                mode: Clutter$d.AnimationMode.EASE_IN_QUAD,
            });
        }
        this.panel.toggle();
    }
...
let MonitorContainer = class MonitorContainer extends St$6.Widget {
    _init(monitor, bgGroup, params) {
        ...
        const horizontalPanelPositionSignal = Me$i.msThemeManager.connect('horizontal-panel-position-changed', () => {
            this.updateSpacer();
        });
        this.connect('destroy', () => {
            Me$i.msThemeManager.disconnect(panelSizeSignal);
            Me$i.msThemeManager.disconnect(horizontalPanelPositionSignal);
            if (this.bgManager) {
    ...
    setFullscreen(monitorIsFullscreen) {
        ...
        this.horizontalPanelSpacer.visible =
            Me$i.layout.panelsVisible && !monitorIsFullscreen;
    }
    ...
    updateSpacer() {
        const panelHeight = Me$i.msThemeManager.getPanelSize(this.monitor.index);
        const panelPosition = Me$i.msThemeManager.horizontalPanelPosition;
    ...
    vfunc_allocate(box, flags) {
        ...
        this.get_children().forEach((actor) => {
            ...
            if (actor === this.msWorkspaceActor) {
                const msWorkspaceActorBox = new Clutter$d.ActorBox();
...
let PrimaryMonitorContainer = class PrimaryMonitorContainer extends MonitorContainer {
    _init(monitor, bgGroup, params) {
        ...
        this.workspaceContainer = new St$6.Widget({
            layout_manager: new Clutter$d.BinLayout(),
            x_align: Clutter$d.ActorAlign.FILL,
            y_align: Clutter$d.ActorAlign.FILL,
        });
        ...
        const verticalPanelPositionSignal = Me$i.msThemeManager.connect('vertical-panel-position-changed', () => {
            this.queue_relayout();
            this.updateSpacer();
        });
        this.connect('destroy', () => {
            Me$i.msThemeManager.disconnect(verticalPanelPositionSignal);
        });
        this.updateSpacer();
    }
    setFullscreen(monitorIsFullscreen) {
        this.panel.visible = Me$i.layout.panelsVisible && !monitorIsFullscreen;
    ...
    setTranslation(prevActor, nextActor) {
        if (!this.translationAnimator.get_parent()) {
            ...
            this.translationAnimator.height = assertNotNull(Main$b.layoutManager.primaryMonitor).height;
            this.workspaceContainer.add_child(this.translationAnimator);
        }
        const indexOfPrevActor = Me$i.msWorkspaceManager.primaryMsWorkspaces.findIndex((msWorkspace) => {
            return msWorkspace.msWorkspaceActor === prevActor;
        });
        const indexOfNextActor = Me$i.msWorkspaceManager.primaryMsWorkspaces.findIndex((msWorkspace) => {
    ...
    updateSpacer() {
        ...
        if (this.verticalPanelSpacer) {
            const panelWidth = Me$i.msThemeManager.getPanelSize(this.monitor.index);
            const panelPosition = Me$i.msThemeManager.verticalPanelPosition;
    ...
    vfunc_allocate(box, flags) {
        ...
        const themeNode = this.get_theme_node();
        box = themeNode.get_content_box(box);
        const panelBox = new Clutter$d.ActorBox();
        const panelPosition = Me$i.msThemeManager.verticalPanelPosition;
        if (this.panel) {
        ...
        if (this.panel && this.panel.visible && Me$i.layout.panelsVisible) {
            if (panelPosition === VerticalPanelPositionEnum.LEFT) {
                msWorkspaceActorBox.x1 =
                    msWorkspaceActorBox.x1 +
                        Me$i.msThemeManager.getPanelSize(Main$b.layoutManager.primaryIndex);
            }
            else {
...
let BaseTilingLayout = class BaseTilingLayout extends Clutter$c.LayoutManager {
    _init(msWorkspace, state = {}) {
        super._init();
        this.lastObservedTileableList = [];
        this._state = Object.assign({}, this.constructor.state, state);
        this.icon = Gio$6.icon_new_for_string(`${Me$h.path}/assets/icons/tiling/${this._state.key}-symbolic.svg`);
        this.msWorkspace = msWorkspace;
        this.themeSettings = getSettings('theme');
        this.signals = [];
        this.registerToSignals();
    ...
    onTileableListChanged(tileableList) {
        ...
        for (const tileable of leavingTileableList) {
            if (tileable instanceof MsWindow &&
                Me$h.msWindowManager.msWindowList.includes(tileable)) {
    ...
    applyGaps(x, y, width, height, screenGap, gap) {
        gap = gap || Me$h.layoutManager.gap;
        if (screenGap == undefined) {
            screenGap = Me$h.layoutManager.useScreenGap
...
let BaseResizeableTilingLayout = class BaseResizeableTilingLayout extends BaseTilingLayout {
    _init(msWorkspace, state) {
        ...
        this.currentFocusEffect = Me$g.msThemeManager.focusEffect;
        this.onGapChange();
    }
    registerToSignals() {
        super.registerToSignals();
        this.signals.push({
            from: Me$g.layoutManager,
            id: Me$g.layoutManager.connect('gap-changed', this.onGapChange.bind(this)),
        }, {
            from: Me$g.msThemeManager,
            id: Me$g.msThemeManager.connect('focus-effect-changed', this.onFocusEffectChanged.bind(this)),
        });
    }
    get state() {
    ...
    onGapChange() {
        if (!Me$g.layoutManager.someGap) {
            if (!this.borderContainer) {
                this.borderActorList = [];
                this.borderContainer = new Clutter$b.Actor();
    ...
    applyGaps(x, y, width, height) {
        const gap = Me$g.layoutManager.gap || BORDER_WIDTH;
        const screenGap = Me$g.layoutManager.useScreenGap
            ? Me$g.layoutManager.screenGap
            : Me$g.layoutManager.gap;
    ...
    addUnFocusEffect(tileable, effect, focused) {
        ...
        if (effect === FocusEffectEnum.DEFAULT) {
            const dimmer = new Clutter$b.BrightnessContrastEffect({
                name: 'dimmer',
                brightness: focused
                    ? Clutter$b.Color.new(127, 127, 127, 255)
                    : Clutter$b.Color.new(100, 100, 100, 255),
            });
        ...
        if (effect === FocusEffectEnum.DEFAULT) {
            if (!focused) {
                ...
                if (tileable.get_effect('dimmer')) {
                    tileable.ease_property('@effects.dimmer.brightness', Clutter$b.Color.new(100, 100, 100, 255), {
                        mode: Clutter$b.AnimationMode.EASE_OUT_QUAD,
                        duration: 150,
                    });
                }
            }
                ...
                if (tileable.get_effect('dimmer')) {
                    tileable.ease_property('@effects.dimmer.brightness', Clutter$b.Color.new(127, 127, 127, 255), {
                        duration: 150,
                        mode: Clutter$b.AnimationMode.EASE_OUT_QUAD,
                        onComplete: () => {
...
let ResizableBorderActor = class ResizableBorderActor extends St$5.Widget {
    _init() {
        ...
        this.set_background_color(new Clutter$b.Color({ red: 10, green: 10, blue: 10, alpha: 255 }));
        this.connect('event', (actor, event) => {
            const eventType = event.type();
            switch (eventType) {
                case Clutter$b.EventType.BUTTON_PRESS:
                case Clutter$b.EventType.TOUCH_BEGIN: {
                    const border = this.portionBorder;
                    assert(border !== undefined, 'Actor has no border');
                    Me$g.msWindowManager.msResizeManager.startResize(border);
                    break;
                }
                case Clutter$b.EventType.ENTER:
                    Me$g.msThemeManager.setCursor(Meta$6.Cursor.MOVE_OR_RESIZE_WINDOW);
                    break;
                case Clutter$b.EventType.LEAVE:
                    Me$g.msThemeManager.setCursor(Meta$6.Cursor.DEFAULT);
                    break;
            }
        });
    }
...
let PrimaryBorderEffect = class PrimaryBorderEffect extends Clutter$b.Effect {
    _init(params) {
        super._init(params);
        this._pipeline = null;
    ...
    vfunc_paint_node(node, paintContext) {
        ...
        if (this.color !== Me$g.msThemeManager.primaryColor) {
            this.color = Me$g.msThemeManager.primaryColor;
...
let MaximizeLayout = class MaximizeLayout extends BaseTilingLayout {
    ...
    startTransition(nextActor, prevActor) {
        ...
        this.translationAnimator.setTranslation([prevActor].filter(isNonNull), [nextActor], indexOfNextActor > indexOfPrevActor ? 1 : -1);
    }
    endTransition() {
...
let MatNumberPicker = class MatNumberPicker extends St$4.BoxLayout {
    _init(value, params) {
        super._init({
            y_align: Clutter$9.ActorAlign.CENTER,
        });
        this.value = value;
...
let SplitLayout = class SplitLayout extends BaseResizeableTilingLayout {
    ...
    refreshVisibleActors() {
        if (this.translationAnimator.animationInProgress)
            return;
    ...
    onFocusChanged(tileableFocused, _oldTileableFocused) {
        ...
        baseIndex = Math.min(baseIndex, this.msWorkspace.tileableList.length - this._state.nbOfColumns);
        baseIndex = Math.max(baseIndex, 0);
        const oldTileableList = this.activeTileableList;
        if (baseIndex !== this.baseIndex) {
            this.baseIndex = baseIndex;
            this.activeTileableList = this.msWorkspace.tileableList.slice(baseIndex, baseIndex + this._state.nbOfColumns);
    ...
    startTransition(previousTileableList, nextTileableList) {
        ...
        const prevBase = previousTileableList.length > 0
            ? this.msWorkspace.tileableList.indexOf(previousTileableList[0])
            : -1;
        const nextBase = nextTileableList.length > 0
            ? this.msWorkspace.tileableList.indexOf(nextTileableList[0])
            : -1;
        const direction = prevBase > nextBase ? -1 : 1;
        const itemWidth = Math.round(this.vertical ? width : width / this._state.nbOfColumns);
        const itemHeight = Math.round(this.vertical ? height / this._state.nbOfColumns : height);
...
class LayoutManager extends MsManager {
    ...
    tileWindows() {
        ...
        GLib$a.idle_add(GLib$a.PRIORITY_DEFAULT, () => {
...
class MsWindowManager extends MsManager {
        ...
        for (const [groupKey, windowActorGroup,] of groupedMetaWindowsByApp.entries()) {
            ...
            for (let i = 0; i < assignments.length; i++) {
                ...
                if (idx < candidateMsWindows.length) {
                    ...
                    logInfoOnce();
                    Me$8.log(`Creating a new window for ${buildMetaWindowIdentifier(windowActor.metaWindow)}`);
    ...
    createNewMsWindow(source, msWorkspace, persistent, initialAllocation, matchingInfo) {
...
let HorizontalPanel = class HorizontalPanel extends St$2.BoxLayout {
    _init(msWorkspace) {
        ...
        const horizontalChangedSignal = Me$6.msThemeManager.connect('clock-horizontal-changed', () => {
        ...
        this.connect('destroy', () => {
            Me$6.msThemeManager.disconnect(horizontalChangedSignal);
        });
        if (Me$6.msThemeManager.clockHorizontal) {
            this.createClock();
        }
